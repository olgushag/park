...
Функция УдалитьЛидирующиеНули(Значение)
	ОбрабатываемаяЧасть = Значение;
	Пока СтрНачинаетсяС(ОбрабатываемаяЧасть,"0") Цикл
		ОбрабатываемаяЧасть = Сред(ОбрабатываемаяЧасть, 2);
	КонецЦикла;
	Возврат ОбрабатываемаяЧасть;
КонецФункции
